...
export const getApplicationEmbed = (
  ...
  const embed = new EmbedBuilder()
    .setTitle(member.displayName + "'s application")
  ...
  try {
    ...
    // Logger.debug(applicationDuration);
    const minutes = Math.floor(applicationDuration / 60);
    const seconds = applicationDuration % 60;
    const timeOnServer = now.diff(dayjs(member.joinedAt), 'day');
    ...
    stats =
      ...
      `**Time on Server:** ${timeOnServer} days\n` +
